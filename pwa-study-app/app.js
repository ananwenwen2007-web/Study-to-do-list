...
const Store = {
  ...
  getReadingCounts() { return this.get('reading_counts', {}); },
  saveReadingCounts(counts) { this.set('reading_counts', counts); },
  getReadingTarget() { return this.get('reading_target', 3); },
  saveReadingTarget(target) { this.set('reading_target', target); },
};
    ...
    'yl-7a': {
      ...
      units: {
        'U1': {
          ...
          reading: "Hi! I'm Millie, a new student at Sunshine Middle School. I'm in Class 1, Grade 7. I'm 12 years old. I love reading. Here are my new classmates. They're very friendly. Let's meet them. This is Simon. He is tall. He is always full of energy. He is good at sport. He often plays football after school. This is Sandy. She is tall and slim. She has long hair. She is very polite. She likes music and she can play the piano well. This is Daniel. He is not tall. He wears glasses. He is smart and always ready to help others. He is interested in science."
        },
        'U2': {
          title: "Unit 2 Let's play sports!",
          reading: "Daniel is one of my new friends. He is a fan of science. He thinks science is cool. Science is all around us. Science changes our lives! Daniel often visits the science museum. He has a chance to see all kinds of things and learn a lot. It is very interesting for him. At home, he likes to take things apart and study how they work. Now he can help fix things at home! In his free time, Daniel reads many science books. He also goes online to find answers to his endless questions. He wants to be a scientist one day. I'm sure his dream will come true."
        },
        'U3': {
          title: 'Unit 3 Welcome to our school!',
          reading: "Hello everyone! I'm Liu Jie from Grade 9. Welcome to our school! This is our classroom building. It is tall and bright. We have lessons here every day. Next to it is the library. We can read books and do homework there. Behind the library is the sports field. We play football and basketball there. Our school is not big, but it is very beautiful. I love my school."
        },
        'U4': {
          title: 'Unit 4 My day',
          reading: "I get up at six thirty every morning. After breakfast, I go to school. Our lessons start at eight o'clock. We have four lessons in the morning and two in the afternoon. After school, I often play basketball with my classmates. Sometimes I go to the library. I go home at five thirty. In the evening, I do my homework and read books. I go to bed at nine thirty. This is my day."
        },
        'U5': {
          title: 'Unit 5 A healthy lifestyle',
          reading: "A healthy lifestyle is important for everyone. We should eat more fruit and vegetables. They are good for our health. We should also drink enough water every day. Exercise is important too. We should do sports for at least thirty minutes every day. Sleep is also important. We should sleep for eight to nine hours every night. A healthy lifestyle helps us study better and live happier."
        },
        'U6': {
          title: 'Unit 6 Fashion',
          reading: "Fashion is all around us. Different people like different styles of clothes. Some people like casual clothes. They are comfortable and easy to wear. Some people like formal clothes. They look smart and elegant. When we choose clothes, we should think about the occasion. We wear different clothes for different activities. Fashion is not just about clothes. It is also about how we feel and how we express ourselves."
        },
        'U7': {
          title: 'Unit 7 Shopping',
          reading: "Shopping is part of our daily life. We buy food, clothes, books and many other things. When we go shopping, we should make a list first. This helps us remember what we need. We should also compare prices and choose the best value. Sometimes we can find good deals on sale. But we should not buy things we don't need just because they are cheap. Smart shopping helps us save money."
        },
        'U8': {
          title: 'Unit 8 Chinese New Year',
          reading: "Chinese New Year is the most important festival in China. It usually comes in January or February. Before the festival, people clean their houses and buy new clothes. On New Year's Eve, families get together and have a big dinner. We eat dumplings and fish. Children get red packets with lucky money. We visit relatives and friends during the festival. Everyone is happy and excited. Chinese New Year is a time for family and celebration."
        }
      }
    }
  },
...
function initSettings() {
  const target = Store.getReadingTarget();
  const input = document.getElementById('readingTargetInput');
  const display = document.getElementById('currentReadingTarget');
  const btnSave = document.getElementById('btnSaveReadingTarget');
  ...
  if (btnSave) {
    btnSave.addEventListener('click', () => {
      ...
      if (newTarget >= 1 && newTarget <= 10) {
        Store.saveReadingTarget(newTarget);
        if (display) display.textContent = newTarget;
        showToast(`朗读遍数已设置为${newTarget}遍`);
      } else {
        showToast('请输入 1-10 之间的数字');
      }
    });
  }
  ...
  try {
    ...
    const detectSound = () => {
      ...
      if (average > 30) { // 音量阈值
        ...
        if (hasSound) {
          totalSoundDuration += (Date.now() - soundStartTime) / 1000;
          hasSound = false;
        }
      }
      ...
      const statusEl = document.getElementById('recordStatus');
      if (statusEl) {
        const currentDuration = hasSound ? totalSoundDuration + (Date.now() - soundStartTime) / 1000 : totalSoundDuration;
        statusEl.textContent = `🔴 录音中...有效朗读${Math.floor(currentDuration)}秒 (目标${MIN_SOUND_DURATION}秒)`;
      }
    ...
    mediaRecorder.onstop = () => {
      ...
      if (totalSoundDuration < MIN_SOUND_DURATION) {
        if (statusEl) statusEl.textContent = ` 朗读时长不足${MIN_SOUND_DURATION}秒，请重新朗读`;
        showToast(`朗读时长不足，请至少朗读${MIN_SOUND_DURATION}秒`);
        return;
      }
      ...
      // 更新朗读次数
      if (taskId) {
        const counts = Store.getReadingCounts();
        counts[taskId] = (counts[taskId] || 0) + 1;
        Store.saveReadingCounts(counts);
        
        const target = Store.getReadingTarget();
        if (counts[taskId] >= target) {
          completeTaskById(taskId, `朗读${counts[taskId]}遍完成`);
          showToast(`🎉 朗读${target}遍完成！积分已发放`);
        } else {
          showToast(`已完成${counts[taskId]}/${target}遍，继续加油！`);
        }
        updatePointsBadge();
      }
    };
